...
# --- Initialize Session State to remember when the plot is unlocked ---
if 'plot_unlocked' not in st.session_state:
    st.session_state.plot_unlocked = False
...
with left_pane:
    ...
    # --- Submit Trigger to Check Sections 1, 2, and 3 ONLY ---
    st.write("")
    generate_plot_btn = st.button("Initialize Telemetry & Plot Trajectory", type="primary", use_container_width=True)

with right_pane:
    # Logic: Validate Phase 1 (Sections 1-3)
    if generate_plot_btn:
        ...
        if math_correct and eq1_correct and eq2_correct and mcq_selection == "Motor 2":
            st.session_state.plot_unlocked = True
        else:
            st.session_state.plot_unlocked = False
            st.error("Diagnostic Error: Invalid Parameters Detected in Sections 1-3. Please verify constraints.")
    ...
    # Logic: Phase 2 - If Plot is Unlocked, Render Plot AND Reveal Section 4
    if st.session_state.plot_unlocked:
        ...
        # --- PART 4: Load Profiling (Revealed After Plot) ---
        st.markdown('<div class="section-label" style="margin-top: 0px;">4. Speed at Specific Loads (rad/s)</div>', unsafe_allow_html=True)
        st.caption("Read the coordinates from the trajectory plot above. Type 'None' if the motor cannot adapt to the profile.")
        ...
        # Second button specifically for verifying the final answers
        verify_btn = st.button("Verify Operational Speeds", type="primary", use_container_width=True)

        if verify_btn:
            m1_spd_correct = (m1_48.strip().lower() == "none") and (m1_12.strip().lower() == "none")
            m2_spd_correct = (abs(m2_48 - 187) < 2.0) and (abs(m2_12 - 387) < 2.0)

            if m1_spd_correct and m2_spd_correct:
                st.success("✅ Operational Speeds Verified! System Ready.")
            else:
                st.error("⚠️ Operational Speed Error.")
                if not m1_spd_correct: st.info("- Motor 1 Speeds: Does Motor 1 naturally adapt to these profiles? Use 'None'.")
                if not m2_spd_correct: st.info("- Motor 2 Speeds: Re-read the intersection coordinates at exactly 48 Nm and 12 Nm.")

    # Logic: If Plot is NOT unlocked, show Offline Canvas
    else:
        st.markdown("""
        <div style="display: flex; height: 100%; min-height: 650px; align-items: center; justify-content: center; background: radial-gradient(circle at center, #1A202C 0%, #0B0F19 100%); border: 1px dashed #2D3748; border-radius: 16px;">
            <div style="text-align: center; color: #4A5568;">
                ...
                <p>Verify Constraints and Math Equations on the left to initialize.</p>
            </div>
        </div>
        """, unsafe_allow_html=True)